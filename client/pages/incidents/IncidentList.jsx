...
import { list, remove } from './api-incident';
...
export default function MyIncidents() {
    ...
    const deleteIncident = async (incident) => {
        try {
            const jwt = auth.isAuthenticated();
            const data = await remove({ incidentId: incident._id }, { t: jwt.token });
            if (data.error) {
                console.error(data.error);
            } else {
                const updatedIncidents = incidents.filter(item => item._id !== incident._id);
